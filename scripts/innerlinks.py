...
# This is a list of common technologies and concepts (that have no translation)
# that I want to link automatically if some coincidence is met.
...
innerlinks["Bootstrap4"] = (
    f"{innerlinks['Bootstrap']}/docs/4.6/getting-started/introduction/",
)  # Bootstrap 4 spec
...
link_pattern = re.compile(
    r"\[[^\]]*\]\([^\)]+\)"
)  # Pattern for existing markdown links
...
def apply_links(content, innerlinks):
    ...
    for i in range(len(chunks)):
        for key, value in innerlinks.items():
            chunks[i] = re.sub(
                r"\b" + re.escape(key) + r"\b", f"[{key}]({value})", chunks[i]
            )

    # Interleave the chunks and link_parts to get the final content
    final_content = "".join(
        val for pair in zip(chunks, link_parts + [""]) for val in pair
    )
...
if __name__ == "__main__":
    ...
    if os.path.exists(root_dir) and os.path.isdir(root_dir):
        print("The content folder exists!")
        confirm = input(
            f"Are you sure you want to permanently delete the directory at {root_dir}? (y/N): "
        )
        if confirm.lower() == "y":
