...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Convert, rescale, and crop an image to a specified size from the center."
    )
    parser.add_argument("origin_image", help="The path to the input image")
    parser.add_argument(
        "destination_image",
        help="The path to save the cropped output image (in .jpg format)",
    )
    parser.add_argument(
        "--crop_width",
        type=int,
        default=440,
        help="The width of the cropped area (default: 440)",
    )
    parser.add_argument(
        "--crop_height",
        type=int,
        default=440,
        help="The height of the cropped area (default: 440)",
    )

    args = parser.parse_args()

    crop_and_resize(
        args.origin_image, args.destination_image, args.crop_width, args.crop_height
    )
